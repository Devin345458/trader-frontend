  ...
  methods: {
    ...
    draw (ctx) {
      ...
      this.$props.data.forEach((p, i) => {
        ...
        const isHover = this.hover(x, y)
        const h = (isHover || this.selected)
        this.highlighted = isHover ? p : null
    ...
    draw_label (ctx, x, y, p, isHover) {
      ctx.fillStyle = isHover ? '#fff' : '#44c667'
    ...
    mousedown (event) {
      if (this.highlighted) {
        if (!this.selected) {
          ...
          // Pull candle data
          const y$ = this.layout.c_magnet(this.selected[0]).raw[2]
